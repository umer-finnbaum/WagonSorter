...
# $env:PLCSIM = "1"
# Set environment variable in powershell and use same session to run Demo script
# PLC simulator must be running.
...
def _read_plc_sim_file():
    """Return dict read from plc_sim.json or None on error."""
    fn = "plc_sim.json"
    try:
...
def get_pair_ids_from_plc():
    # Simulation override: if PLCSIM=1 read values from plc_sim.json
    if os.environ.get("PLCSIM") == "1":
        data = _read_plc_sim_file()
        if data is not None:
            id1 = data.get("Wacon_INT[1]")
            id2 = data.get("Wacon_INT[2]")
...
def get_filename_from_plc():
    ...
    if os.environ.get("PLCSIM") == "1":
        data = _read_plc_sim_file()
        if data is not None:
            s = data.get("Wacon_SerieName")
            return None if s is None or s == "" else str(s)
    with PLC() as comm:
        comm.IPAddress = PLC_IP
        response = comm.Read("Wacon_SerieName")
...
class WagonWidget(QtWidgets.QWidget):
    ...
    def setup_graphics_items(self):
        ...
        wagon_width = self.cfg["width"] * 1.3
...
class StackerApp(QtWidgets.QApplication):
    ...
    def poll_plc_and_save(self):
        ...
        # Handle PLC filename blank logic
        if not plc_filename:
            ...
            if elapsed < self.empty_filename_timeout:
                ...
                if self.current_filename != "":
                    ...
                    if hasattr(self.overview_window, "update_page"):
                        ...
                        self.overview_window.shapes_by_id = {}
                        self.overview_window.slot_occupancy = {}
                        # keep overview series_name in sync when blank
                        self.overview_window.series_name = self._normalize_series_name(self.current_filename)
                        self.overview_window.update_page(highlight_ids=[])
                    self.already_placed_ids = set()
                else:
            ...
            # PLC filename is non-empty, reset timer
            self.empty_filename_start_time = None
        ...
        '''
        curr_ids_display = [str(i) for i in ids if i not in (None, "", "-2")]
        prev_ids = getattr(self, "previous_drawn_ids", (None, None))
        prev_ids_display = [str(i) for i in prev_ids if i not in (None, "", "-2", 0, "0")]'''
        ...
        # Previous ID 1 (most recent placed)
        if prev1:
            pol1, coords1 = _pol_and_coords(prev1)
            status_lines.append(f"1: Previous Part= P{pol1} - {coords1}")
        else:
            status_lines.append("1: Previous Part= -")

        # Previous ID 2 (second most recent placed)
        if prev2:
            pol2, coords2 = _pol_and_coords(prev2)
            status_lines.append(f"2: Previous Part= P{pol2} - {coords2}")
        else:
            status_lines.append("2: Previous Part= -")

        # Previous ID 3 (third most recent placed)
        if prev3:
            pol3, coords3 = _pol_and_coords(prev3)
            status_lines.append(f"3: Previous Part= P{pol3} - {coords3}")
        else:
            status_lines.append("3: Previous Part= -")

        # Previous ID 4 (fourth most recent placed)
        if prev4:
            pol4, coords4 = _pol_and_coords(prev4)
            status_lines.append(f"4: Previous Part= P{pol4} - {coords4}")
        else:
            status_lines.append("5: Previous Part= -")
        ...
        '''# If no IDs, show status only and save file
        if not (id1 or id2):
        ...
        '''# Only update drawn ids if new ids are different
        if any(ids) and ids != getattr(self, 'last_drawn_ids', None):
            self.previous_drawn_ids = getattr(self, 'last_drawn_ids', (None, None))
            self.update_main_window(ids)
